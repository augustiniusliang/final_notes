...
\begin{problem}
    计算以下反常积分:
    \[
        \int_0^{\infty} \frac{dx}{(1+x^2)^n}, \quad \int_0^{\infty} \frac{dx}{(x^2 + ax + b)^n} (4b > a^2).
    \]
\end{problem}
...
\begin{problem}
    ...
    \[
        \left( \int_0^1 xf(x) \, dx \right)^2 \leq \frac{1}{45} \int_0^1 (f'(x))^2 \, dx.
    \]
\end{problem}
...
\begin{problem}
    证明: 对于下凸函数 $f(x)$, 有
    \[
        f\left(\int_a^b p(x)  \, dx\right) \leq \int_a^b f(p(x)) \, dx.
    \]
